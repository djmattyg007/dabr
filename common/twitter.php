	...
	switch(intval($response_info['http_code'])) {
		...
		default:
			...
			*/	
				...
				theme('error', "<h2>Status was toooo looong!</h2><p>{$status}</p><hr>");
...
function twitter_confirmation_page($query)
{
	...
	switch ($action) {
		...
		case "hideretweets":
			...
			$content .= "<ul><li>Their retweets will no longer appear in your timeline.</li><li>You will still see them when looking at the timeline of $target.</li></ul>";
			break;
	}
...
function theme_directs_form($to)
{
	...
	if (setting_fetch("lastDM", "") == "yes") {
		...
		$htmlTo .= "<b>" . $DM[0]->sender_screen_name . "</b><br />" . nl2br($DM[0]->text) . "<br /><br />";
	}
	if ($to) {
		...
		$htmlTo .= "To: <input name='to'><br />Message:";
	}
	$content = "<form action='directs/send' method='post'>$htmlTo<br><textarea name='message' style='width:90%; max-width: 400px;' rows='3' id='message'></textarea><br><input type='submit' value='Send'><span id='remaining'>140</span></form>";
...
function twitter_find_tweet_in_timeline($tweetID, $tl)
{
	// Parameter checks
	if (!is_numeric($tweetID) || !$tl) {
		return;
	}

	// Check if the tweet exists in the timeline given
	if (array_key_exists($tweetID, $tl)) {
		// Found the tweet
		$tweet = $tl[$tweetID];
	} else {
		// Not found, fetch it specifically from the API
		$request = API_NEW . "statuses/show.json?id={$tweetID}";
...
function twitter_home_page()
{
	user_ensure_authenticated();
	$perPage = setting_fetch('perPage', 20);
	$request = API_NEW . "statuses/home_timeline.json?count={$perPage}";
	if ($_GET['max_id']) {
		$request .= '&max_id=' . $_GET['max_id'];
	}
	if ($_GET['since_id']) {
...
function theme_avatar($url, $forceLarge = false)
{
	$size = $forceLarge ? 48 : 24;
...
function theme_timeline($feed, $paginate = true)
{
	...
	foreach ($feed as $status) {
		...
		//Build main HTML string.
		//$html = "<b><a href='user/{$status->from->screen_name}'>{$status->from->screen_name}</a></b> $actions $link<br />{$text}<br />$media<small>$source</small>";
		$html = "<b><a href='user/{$status->from->screen_name}'>{$status->from->screen_name}</a></b> $actions $link<br />{$text}<br />$media<span class='from'>$source</span>$retweeted";
	...
	if (!$hide_pagination) {
		if ($paginate) {
			...
			//if ($page == '' || $page == 'user' || $page == 'search' || $page == 'hash' || $page == 'tofrom' || $page == 'replies' || $page == 'directs') {
				...
				if (is_64bit()) {
					$max_id = intval($max_id) - 1; //stops last tweet appearing as first tweet on next page
				}
...
function theme_pagination($max_id = false)
{
	...
	if ($max_id) {
		...
		if ($page > 1) {
			$links[] = "<a href='{$_GET['q']}?page=" . ($page-1) . "$query' accesskey='8'>Newer</a> 8";
		}
	}
	if ($query) {
